const ATTACKS = [
  {
    id: "lightning",
    name: "Lightning",
    description: "Narrow strike, deep penetration",
    speed: 470,
    spread: 3,
    radius: 8,
    impactWidth: 22,
  ...
  },
  {
    id: "sunbeam",
    name: "Sun beam",
    description: "Medium width, scorching penetration",
    speed: 390,
    spread: 13,
    radius: 15,
    impactWidth: 28,
...
function updateCloud(now) {
  state.cloud.x = CLOUD.baseX + Math.sin(now * CLOUD.speed) * CLOUD.driftX;
...
function startSuperRain(side, chosenAngle, attackIndex) {
  ...
  state.weatherEffect = {
    ...
    erosionTimer: 0,
    waterHeight: 0,
    maxWaterHeight: attack.floodHeight,
...
function updateWeatherEffect(deltaSeconds) {
  ...
  effect.waterHeight = attack.floodHeight * progress;
...
function updateLingeringEffects(deltaSeconds) {
  state.lingeringEffects = state.lingeringEffects
    .map((effect) => ({
      ...effect,
...
function maybeEnemyFire(now) {
  if (state.turn !== "enemy" || state.projectile || state.winner || now < state.pendingEnemyShotAt) {
    return;
  }
...
function drawCastle(side) {
  ...
  const stoneBase = side === "player" ? "#7bccef" : "#efbb73";
  const stoneShade = side === "player" ? "#3f86ae" : "#b86c38";
  const mortar = side === "player" ? "rgba(219, 247, 255, 0.45)" : "rgba(255, 234, 196, 0.42)";
  const trim = side === "player" ? "#d7f6ff" : "#ffe6bb";
  const banner = side === "player" ? "#4dd8ff" : "#ff8f43";
  ...
  baseGradient.addColorStop(0, side === "player" ? "#4e89a8" : "#a3673c");
  baseGradient.addColorStop(1, side === "player" ? "#2d5364" : "#6f4527");
  ...
  drawGate(gateX, gateY, 42, 58, side === "player" ? "#214659" : "#5f3416", trim);
...
function drawDetailedBlock(block, mortar, trim) {
  ...
  ctx.beginPath();
  ctx.moveTo(block.x + 2, block.y + 3);
  ctx.lineTo(block.x + block.width - 3, block.y + 3);
  ctx.stroke();

  ctx.strokeStyle = "rgba(0, 0, 0, 0.18)";
  ctx.beginPath();
  ctx.moveTo(block.x + 3, block.y + block.height - 3);
  ctx.lineTo(block.x + block.width - 2, block.y + block.height - 3);
  ctx.stroke();
  ...
  ctx.moveTo(block.x + 2, block.y + (block.height / 2));
  ctx.lineTo(block.x + block.width - 2, block.y + (block.height / 2));
  ctx.stroke();
...
function drawHudText() {
  ...
  ctx.fillRect(332, 18, 296, 48);
  ctx.fillStyle = "#ffffff";
  ctx.font = "18px Georgia";
  ctx.fillText(`Attack: ${ATTACKS[state.selectedAttack].name}`, 350, 38);
  ctx.fillText(`Angle: ${state.angle}°`, 350, 58);
}
